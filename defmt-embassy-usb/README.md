...
`embassy-usb` driver implementation (RP2040/RP2350, STM32, nRF52, ESP32, …).
...
# Cargo.toml
[dependencies]
defmt-embassy-usb = { version = "0.0.1", features = ["global-logger"] }
```
...
let defmt_task = defmt_embassy_usb::UsbDefmtLogger::new()
...
```rust
type DefmtTask = defmt_embassy_usb::UsbDefmtTask<YourDriver>;
...
```toml
defmt-embassy-usb = { version = "0.1" }   # no global-logger feature
```
...
fn sink(bytes: &[u8]) {
    ...
    unsafe { defmt_embassy_usb::write(bytes); }
}

unsafe fn flush() {
    // SAFETY: inside the defmt critical section.
    unsafe { defmt_embassy_usb::swap(); }
}
```
